...
export const usePlantStore = create<PlantState>((set) => ({
  ...
  addPlant: (newPlant) => set((state) => actions.addPlantLogic(state, newPlant)),

  deletePlant: (plantId) => set((state) => actions.deletePlantLogic(state, plantId)),

  waterPlant: (plantId, newDate) => set((state) => actions.waterPlantLogic(state, plantId, newDate)),

  updatePlant: (updatedPlant) => set((state) => actions.updatePlantLogic(state, updatedPlant)),
}))
